...
some terms to be introduced to be often use in networkplots

*Links/Edges	
A data frame object with the links between the nodes. It should include the Source (from) and Target (to) for each link, as well as other properties (depending on package being use)

*Nodes/Vertices	 (Address, Enties, Intermediaries, Officers in this case)
A data frame containing the node id and properties of the nodes.

The .csv are also rather well structured to be adapted into network packages, Entities, Addresses, Intermediaries, and Officers datasets llisting are all attributed to a "node_id", while the "node_1" and "node_2" column in Edge.csv seems to be describing the relationship between said nodes.
...
While there is already some nodes depicting of the country names and codes as "Not Identify", some of the countries column in the node are left emtpy/NA, I am not sure if there are any significant associated with "Not Identify". 

Hence I  am going to attributes these listing of nodes without countries(empty/NA) with countries= "Unknown", country_codes = "XXX".
...
some of the countries names are filled with names of multiple countries, such as 
"British Virgin Islands;Hong Kong", instead of a single name. These countries identity can be seperated with the ";"


For the single listing country 
...
* Aethestically pleasing, among the cleanest network plot

Issues
* No direct method to overlay the nodes onto of a map,
While it is possible to force them into respective coordintes(x, y) and disable Physics, to properly display them would require some work on scaling.
...
### Network d3
[githubpage/document](https://github.com/christophergandrud/d3Network)
